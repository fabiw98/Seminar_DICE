...
In diesem Kapitel wird DiCE als ein Werkzeug betrachtet, um bestehende Probleme in der Praxis zu lösen. Die Anwendung von DiCE wird anhand eines konkreten Beispiel demonstriert.
...
Diese Handlungsfähigkeit wird durch die Generierung von Counterfactuals gegeben. Sie liefern dem Anwender mehrere "Was-wäre-wenn"\--Szenarien. Aufgrund der Diversität der Erklärungen stehen dem Anwender verschiedene Vorschläge zum Anpassen zur Verfügung und erhöhen so die Umsetzbarkeit von mindestens einer Möglichkeit.
% Zielgruppen: Anwender, Auditoren und Entwickler
Der Einsatz von DiCE ist nicht nur auf den Endanwender beschränkt, sondern kann auch zum Aufdecken von Bias in Modellen verwendet werden. Dies ermöglicht einen breiten Einsatz des Erklärungsansatzes. 
...
\begin{table}[h]
	\centering
	\caption{Der Eingabevektor (Zeile 0) aus dem Testset, für den Counterfactuals generiert werden sollen.}
	\label{tab:cf_input}
	\small
...
\begin{table}[h]
	\centering
	\caption{Das diverse Set an generierten Counterfactuals. Es werden nur die Features angezeigt, die sich vom Eingabevektor unterscheiden.}
	\label{tab:cf_output}
	\small
	\begin{tabular}{|c | c |c |c |c |c |c |c |c|}
		\hline
...
Es ist bei den Einschränkungen zu beachten, dass zu viele Limitierungen eine Generierung von Erklärungen verhindern kann. Wird der Familienstand in dem obigen Beispiel als unveränderlich betrachtet, so findet DiCE keine Counterfactuals mehr. Dieses Phänomen kann auf einen Bias im Datensatz $adult$ hindeuten, da die weibliche Person unter den angebenden Einschränkungen aus Tabelle \ref{tab:cf_input} nicht ohne Heirat in die höhere Einkommensklasse aufsteigen kann.


\subsection{Evaluation des Beispiels}
Unter Verwendung der Metriken aus Kapitel 4 lassen sich die beschränkten und unbeschränkten Counterfactuals aus den Tabellen \ref{tab:cf_output} und \ref{tab:cf_output_optimized} in Tabelle \ref{tab:evaluation_comparison} bewerten und vergleichen. Neben der Gültigkeit aller Counterfactuals sind ebenfalls die gleichbleibenden Werte für die kategoriale Proximität und Sparsität hervorzuheben. Die lässt sich darin begründen, dass für die beiden CF-Mengen im Durchschnitt zwei der sechs kategorialen Features geändert wurden. Die gleiche Argumentation gilt für die Sparsität. Auf der anderen Seite zeigt die kontinuierliche Proximität große Unterschiede zwischen den beiden CF-Mengen. Der hohe Wert der unbeschränkten Counterfactuals ist das Resultat aus der starken Erhöhung des Alters und der Wochenarbeitsstunden. Da diese Features bei der zweiten Generierung limitiert wurden, weisen die beschränkten Counterfactuals eine größere Nähe zu der ursprünglichen Eingabe auf. Aus der gestiegenen Proximität durch die Beschränkungen folgt allerdings eine sinkende Diversität, insbesondere für die kontinuierlichen Features. Dies ist zu erwarten, da die möglichen Variationen der ursprünglichen Eingabe stark eingeschränkt wurden, sodass sich die generierten Counterfactuals sehr ähnlich sind. Die Evaluationsergebnisse der Demonstration zeigen die Grenzen von DiCE und die konkurrierenden Ziele zwischen Diversität und Proximität auf.
\begin{table}[h]
	\centering
	\caption{Vergleich der Evaluationsergebnisse für unbeschränkte (Tabelle \ref{tab:cf_output}) und beschränkte (Tabelle \ref{tab:cf_output_optimized}) Counterfactuals.}
	\label{tab:evaluation_comparison}
	\begin{tabular}{|l|c|c|}
		\hline
		\textbf{Evaluationsmetrik} & \textbf{Unbeschränkte CFs} & \textbf{Beschränkte CFs} \\ \hline
		Gültigkeit (\%ValidCFs) & 1,0000 & 1,0000 \\ \hline
		Kontinuierliche Proximität & -6,7125 & -2,9500 \\ \hline
		Kategoriale Proximität & 0,6667 & 0,6667 \\ \hline
		Sparsität & 0,6250 & 0,6250 \\ \hline
		Kontinuierliche Diversität & 6,8417 & 0,4333 \\ \hline
		Kategoriale Diversität & 0,2500 & 0,2778 \\ \hline
		Count Diversität & 0,3750 & 0,3125 \\ \hline
	\end{tabular}
\end{table}
