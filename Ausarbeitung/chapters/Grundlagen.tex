...
\subsection{Modell des maschinellen Lernens}
Im Kontext dieser Arbeit wird ein \textbf{Modell des maschinellen Lernens} als eine Funktion betrachtet, die Eingabedaten einer bestimmten Kategorie zuordnet. Die Eingabe erfolgt in Form eines \textbf{Feature-Vektors}, welcher die numerischen und kategorialen Merkmale eines Objekts zusammenfasst. Das Ergebnis der Verarbeitung ist die Zuordnung zu einer diskreten \textbf{Klasse}, wie in Abbildung \ref{fig:ml-prozess} exemplarisch gezeigt wird. 
...
Eine \textbf{kontrafaktische Erklärung}, auch Counterfactual (CF) genannt, ist ein modifiziertes Gegenbeispiel zur ursprünglichen Eingabe, welches so gewählt wird, dass das Modell eine \textbf{erwünschte Klasse} (CF-Klasse) ausgibt. 
Das Ziel eines von DiCE generierten Counterfactuals ist es, dem Anwender die minimal notwendigen Änderungen aufzuzeigen, um ein gewünschtes Ergebnis zu erzielen. Ein Beispiel für eine Erklärung ist in Abbildung \ref{fig:ml-prozess-cf} gegeben. Im Vergleich zur ursprünglichen Eingabe (Abb. \ref{fig:ml-prozess}) wird dem Anwender aufgezeigt, dass bei einem höheren Einkommen und einer Anstellung als Beamter der gewünschte Kredit gewährt wird.\cite{mothilal2020dice}
\begin{figure}[htbp]
	\centering
	\resizebox{\textwidth}{!}{
	\begin{tikzpicture}[
